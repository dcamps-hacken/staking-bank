...
const R1 = reward.mul(2).div(10);
const R2 = reward.mul(3).div(10);
  ...
  : describe("Bank unit testing", function () {
      ...
      describe("withdraw", function () {
        ...
        it("emits the Withdraw event with args", async function () {
          ...
          await expect(bank.connect(user1).withdraw())
            ...
            .withArgs(user1.address, minted, R1);
        });
      });
      ...
      describe("retrieve", function () {
        ...
        it("reverts if the bank is not in third unlock period", async function () {
          // check retrieve is reverted
          await expect(bank.retrieve()).to.be.revertedWith(
            "Retrieve not available yet"
          );
        });
        ...
        it("sends all remaining tokens to the contract owner", async function () {
          // move the timestamp to third unlock: retrieve available and R = R1 + R2 + R3
          await network.provider.request({
            method: "evm_increaseTime",
            params: [this.thirdUnlock],
          });
          ...
          assert.equal(balanceIncrease.toString(), reward.toString());
        });

        it("emits the retrieve event with args", async function () {
          // move the timestamp to third unlock
          await network.provider.request({
            method: "evm_increaseTime",
            params: [this.thirdUnlock],
          });
          ...
          // check emission of retrieve event
          await expect(bank.retrieve())
            .to.emit(bank, "Retrieve")
            .withArgs(reward);
        });
      });
      ...
      describe("getR", function () {
        it("R equals R1 after the first unlock", async function () {
          ...
          assert.equal(R.toString(), R1.toString());
        });
        ...
        it("R equals R1 + R2 after the second unlock", async function () {
          ...
          // get reward tokens R and compare with expected value
          const R = await bank.callStatic.getR();
          assert.equal(R.toString(), R1.add(R2).toString());
        });
